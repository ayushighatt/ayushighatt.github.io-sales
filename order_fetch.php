...
$query .= "
	SELECT * FROM inventory_order INNER JOIN customer ON customer.cust_id = inventory_order.inventory_order_name_id WHERE";


if(isset($_POST["search"]["value"]))
{
	$query .= '(inventory_order_id LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR customer.cust_name LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = '<button type="button" name="delete" id="'.$row["inventory_order_id"].'" class="btn btn-danger btn-xs delete" data-status="'.$row["inventory_order_status"].'">Ship</button>';
	$data[] = $sub_array;
}
