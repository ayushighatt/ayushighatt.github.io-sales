    					...
    					<div class="row">
							<div class="col-md-6">
								...
								<label>Enter Customer Name</label>
								 <select name="inventory_order_name_id" id="inventory_order_name_id" class="form-control" required>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#inventory_order_name_id').change(function(){
        var cust_id = $('#inventory_order_name_id').val();
        var btn_action = 'load_addr';
        $.ajax({
            ...
            {
                $('#inventory_order_address').html(data);
            }
        });
    });
		...
		$(document).on('click', '.update', function(){
			...
			$.ajax({
				...
				{
					$('#orderModal').modal('show');
					$('#inventory_order_name_id').val(data.inventory_order_name_id);
					$('#inventory_order_date').val(data.inventory_order_date);
					$('#inventory_order_address').html(data.addr_select_box);
		...
		$(document).on('click', '.delete', function(){
			...
			if(confirm("Are you sure you want to ship the product?"))
			{
				$.ajax({
					url:"order_action.php",
					method:"POST",
