...
function fill_productss_list($connect, $category_id, $brand_id)
{
...
function fill_cust_addr($connect,$cust_id)
{
$query = "SELECT * FROM customer WHERE cust_id= $cust_id";
	...
	foreach($result as $row)
	{
		$output .= '<option value="'.$row["cust_addr"].'">'.$row["cust_addr"].'</option>';
	}
	return $output;
...
function available_product_quantity($connect, $product_id)
{
	...
	{$query="UPDATE product SET product_quantity=$available_quantity where product_id=$product_id";
	 $statement = $connect->prepare($query);
	$statement->execute();
	return true;
    }
