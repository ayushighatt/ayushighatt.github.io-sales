...
if(isset($_POST['btn_action']))
{
   ...
   if($_POST['btn_action'] == 'load_addr')
	{
		echo fill_cust_addr($connect, $_POST['cust_id']);
	}
	...
	{
		$query = "
		INSERT INTO inventory_order (user_id, inventory_order_total, inventory_order_date, inventory_order_name_id, inventory_order_address, payment_status, inventory_order_status, inventory_order_created_date) 
		VALUES (:user_id, :inventory_order_total, :inventory_order_date, :inventory_order_name_id, :inventory_order_address, :payment_status, :inventory_order_status, :inventory_order_created_date)
		";
		...
		$statement->execute(
			array(
				...
				':inventory_order_name_id'			=>	$_POST['inventory_order_name_id'],
	...
	if($_POST['btn_action'] == 'fetch_single')
	{
		...
		{
			$output['inventory_order_name_id'] = $row['inventory_order_name_id'];
			$output['inventory_order_date'] = $row['inventory_order_date'];
			$output['inventory_order_address'] = $row['inventory_order_address'];
			$output["addr_select_box"] = fill_cust_addr($connect, $row["inventory_order_name_id"]);
	...
	if($_POST['btn_action'] == 'Edit')
	{
		...
		if(isset($delete_result))
		{
			...
			$update_query = "
			UPDATE inventory_order 
			SET inventory_order_name_id = :inventory_order_name_id, 
	...
	{   $order_id=$_POST["inventory_order_id"];
		$p=0;
		$query="SELECT * FROM inventory_order_product INNER JOIN inventory_order on inventory_order.inventory_order_id=inventory_order_product.inventory_order_id
		WHERE inventory_order_product.inventory_order_id=$order_id";
        $statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
		foreach ($result as $row )
		{
		$abc=available_product_quantity($connect, $row['product_id']);
		...
		$result = $statement->fetchAll();
				$p=1;
				foreach ($result as $row )
				echo 'Low stock for product '.$row['product_name'] . ' Cannot be shipped!!!';
			}
		}

        if($_POST['status'] == 'active'  && $p!=1)
		{
			$status = 'inactive';
